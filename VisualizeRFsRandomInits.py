...
for copy in range(10):
    kept_filters = []
    RFs = []
    for filter_index in range(layer_dict[layer_name].output.shape[3]): #layer_dict[layer_name].output.shape[3]
        ...
        if K.image_data_format() == 'channels_first':
            input_img_data = 0.0*np.ones((1, 1, img_width, img_height)) + 0.1*np.random.normal(size=(1, 1, img_width, img_height))
        else:
            input_img_data = 0.0*np.ones((1, img_width, img_height, 1)) + 0.1*np.random.normal(size=(1, img_width, img_height, 1))
        orig_image_data = np.array(input_img_data)
        ...
        for i in range(1):
            ...
            input_img_data -= orig_image_data
    ...
    np.save('saved_filters/RI'+str(copy)+'_'+model_name+'_'+str(layer_name)+'.npy', RFs)
    ...
    imsave('saved_visualizations/RI'+str(copy)+model_name+'_'+str(layer_name)+'.png', stitched_filters)
